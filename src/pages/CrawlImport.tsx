...
export default function CrawlImport({ onImportSuccess, onNavigate }: CrawlImportProps) {
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      setError("Bulk import failed at some point.");
    } finally {
  ...
  const handleCrawl = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Failed to crawl metadata. Please check the App ID.");
      }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setError("Failed to import app into catalog.");
    } finally {
      setImporting(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-utility-text tracking-tight">Crawl & Import</h1>
        <p className="text-xs text-utility-muted mt-0.5">Contact external APIs to fetch and import new application metadata.</p>
      </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={store === "Google Play" ? "e.g. com.whatsapp" : "e.g. 389801252"}
              ...
              <p className="mt-2 text-[11px] text-utility-muted italic">
                Accepts package names, numerical IDs, or full HTTPS store URLs.
              </p>
            </div>
            
            <div className="flex justify-end pt-2">
              <Button type="submit" isLoading={isCrawling} disabled={!appId} className="!px-6">
                <span>Crawl Metadata</span>
              </Button>
            </div>
          </form>
        </div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h2 className="text-[11px] font-bold text-utility-muted uppercase tracking-widest">Browse by Category</h2>
          <div className="flex gap-2">
            {["Communication", "Productivity"].map(cat => (
              <button
                key={cat}
                onClick={() => fetchDiscoveries(cat)}
        ...
        {selectedAppIds.size > 0 && (
          <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}>
            ...
            >
              Import {selectedAppIds.size} Selected
            </Button>
          </motion.div>
        )}
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-[#fafafa] border-b border-utility-border">
                <tr>
                  ...
                  <th className="px-5 py-3 text-[11px] font-bold text-utility-muted uppercase tracking-wider">Application</th>
                  <th className="px-5 py-3 text-[11px] font-bold text-utility-muted uppercase tracking-wider text-right">Preview</th>
                </tr>
              </thead>
      ...
      <AnimatePresence>
        {isCrawling && (
          ...
          >
            ...
            <p className="text-[12px] text-utility-muted">Fetching from {store}...</p>
          </motion.div>
        )}
        ...
        {error && (
          ...
          >
            ...
            <div>
              <p className="text-[12px] font-bold text-red-900">Crawl Failed</p>
        ...
        {preview && !importSuccess && (
          ...
          >
            <div className="bg-[#fafafa] border-b border-utility-border px-5 py-3 flex items-center justify-between">
              <span className="text-[11px] font-bold text-utility-muted uppercase tracking-wider">Store Metadata Result</span>
              ...
              <div className="flex gap-6 items-start">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="block text-[10px] font-bold text-utility-muted uppercase tracking-wider mb-0.5">Version</span>
                      <span className="text-[13px] text-utility-text font-semibold">{preview.version}</span>
                    </div>
                    <div>
                      <span className="block text-[10px] font-bold text-utility-muted uppercase tracking-wider mb-0.5">Category</span>
        ...
        {importSuccess && (
          ...
          >
            ...
            <h2 className="text-xl font-bold text-utility-text mb-2">Successfully Imported</h2>
            <p className="text-sm text-utility-muted max-w-sm mb-8 font-medium">
              The app has been added to the internal catalog. You can now assign it to a group in management.
            </p>
            <Button variant="secondary" onClick={() => onNavigate?.("catalog")}>
              <span>Go to Catalog Management</span>
